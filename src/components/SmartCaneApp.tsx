...
interface Translations {
    ...
    mapTitle: string;
    loadingMap: string;
    voiceWarningTitle?: string;
    voiceWarningMessage?: string;
}
...
const translations: Record<Language, Translations> = {
    fr: {
        ...
        mapOpening: "Affichage de la carte d'Agadir",
        mapTitle: "Carte d'Agadir",
        loadingMap: "Chargement de la carte...",
        voiceWarningTitle: "Avertissement de voix",
        voiceWarningMessage: "La synthèse vocale en arabe peut ne pas être supportée sur tous les navigateurs"
    },
    ar: {
        appTitle: "كين كونكت",
        ...
        mapOpening: "عرض خريكة أكادير",
        mapTitle: "خريطة أكادير",
        loadingMap: "جاري تحميل الخريطة...",
        voiceWarningTitle: "تحذير الصوت",
        voiceWarningMessage: "قد لا يتم دعم الصوت العربي على جميع المتصفحات"
    },
    en: {
        appTitle: "CaneConnect",
        ...
        mapOpening: "Displaying Agadir map",
        mapTitle: "Agadir Map",
        loadingMap: "Loading map...",
        voiceWarningTitle: "Voice Warning",
        voiceWarningMessage: "Arabic voice synthesis may not be supported on all browsers"
    }
};

// Fonction pour vérifier le support de l'arabe
const hasArabicSupport = () => {
    if (typeof window === 'undefined' || !('speechSynthesis' in window)) return false;

    const voices = window.speechSynthesis.getVoices();
    return voices.some(voice => voice.lang.includes('ar'));
};
...
const SmartCaneApp = () => {
    ...
    // Utilisation du hook de notifications vocales simplifié
    const { speak, isVoiceReady } = useVoiceNotifications({
    ...
    const handleSettingsClick = () => {
        ...
        speak(t.accessibilitySettings); // "Paramètres d'accessibilité"
    };
    ...
    const handleLocationClick = () => {
        ...
        switch (currentLanguage) {
            case 'fr':
                message = "Bienvenue à Agadir";
                break;
            case 'ar':
                message = "عرض خريكة أكادير";
                break;
            case 'en':
                message = "Welcome to Agadir";
                break;
            default:
                message = "Displaying Agadir map";
        }
    ...
    const handleLanguageChange = (value: Language) => {
        ...
        // Annoncer la nouvelle langue après un court délai
        setTimeout(() => {
            const newLanguageName = translations[value].languageName;
            speak(newLanguageName);

            // Avertissement si l'arabe n'est pas supporté
            if (value === 'ar' && !hasArabicSupport()) {
                setTimeout(() => {
                    toast({
                        title: translations[value].voiceWarningTitle,
                        description: translations[value].voiceWarningMessage,
                        variant: "default"
                    });
                }, 1000);
            }
        }, 300); // Court délai pour laisser le temps à la langue de changer
    };
    return (
